...
3. 오늘의 표현과 단어를 확인한다.
4. AI와 레벨별 최소/최대 학습자 답변 수 기준으로 대화를 진행한다.
...
3. `topic-taxonomy-spec.md`
   - 성인 학습자용 상위/하위 토픽 카테고리와 레벨별 콘텐츠 제작 표

4. `content-data-schema.md`
   - 시나리오, 미션, 표현, 단어, 대화, 피드백의 데이터 구조

5. `mvp-scenarios-spec.md`
   - MVP 기본 시나리오 5개
...
8. `voice-conversation-spec.md`
   - 마이크, 음성 인식, 키보드 입력, TTS, 번역, 대화 종료 기준

9. `report-metrics-spec.md`
   - 학습 리포트 계산 기준

10. `empty-error-copy-spec.md`
...
- `turnCount`는 완료된 AI-학습자 대화 쌍의 개수이며, 학습자가 실제로 답변한 횟수와 같다.
- 최소 답변 수 전에는 미션이 완료되어도 자동 종료하지 않는다.
- 미션 완료 + 최소 답변 수 이상이면 종료 선택지를 보여준다.
- 사용자가 더 대화하기를 선택하면 해당 레벨의 최대 답변 수까지 확장한다.
- 마무리 예고 시점부터 AI는 자연스럽게 종료를 준비한다.
- 최대 답변 수에 도달하면 최종 발화 후 리포트로 이동한다.
...
12. 0턴 종료
13. 1회 이상 답변 후 종료
14. 미션 완료 + 레벨별 최소 답변 수 이상 종료
15. 더 대화하기
16. 레벨별 마무리 예고
17. 레벨별 최대 답변 수 자동 종료
18. 전체 대화 보기
19. 레벨 변경
20. 말하기 속도 변경
21. 알림 시간 변경
